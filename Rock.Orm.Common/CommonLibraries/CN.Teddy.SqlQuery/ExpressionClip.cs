...
namespace Rock.Orm.Common
{
    public abstract class ExpressionBase : IExpression
    {
        ...
        {
            return CN.Rock.Common.CommonUtils.MakeUniqueKey(15, "p");
        }
            ...
            get
            {
                ...
                while (en.MoveNext())
                {
                    ...
                    else
                    {
                        ...
                        list[i++] = CN.Rock.Common.SerializationManager.Instance.Serialize(en.Current.Value.Value);
                    }
                }

                return CN.Rock.Common.SerializationManager.Instance.Serialize(list);
            }
            set
            {
                ...
                string[] list = (string[])CN.Rock.Common.SerializationManager.Instance.Deserialize(typeof(string[]), value);
                parameters.Clear();
                for (int i = 0; i < list.Length / 4; ++i)
                {
                    ...
                    else
                    {
                        keyValue = new KeyValuePair<DbType,object>((DbType)int.Parse(list[i + 1]),
                            CN.Rock.Common.SerializationManager.Instance.Deserialize(CN.Rock.Common.CommonUtils.GetType(list[i + 2]), list[i + 3]));
                    }
