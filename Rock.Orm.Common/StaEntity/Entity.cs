...
namespace Rock.Orm.Common
{
    ...
    public class Entity : MarshalByRefObject
    {
        ...
        #region BatchSize Changes Monitor
        ...
        /// <param name="propertyName">Name of the property.</param>
        /// <param name="baseEntity">The base entity.不是基类</param>
        /// <returns>The query result.</returns>
        ...
        protected void OnQueryPropertyItemAdd(string propertyName, object item)
        {
            ...
            lock (toDeleteRelatedPropertyObjects)
            {
                ...
                if (toDeleteRelatedPropertyObjects.TryGetValue(propertyName, out values) && values.Contains(item))
                {
                    values.Remove(item);
                    isInDeleteList = true;
                }
        ...
        protected void OnQueryPropertyItemRemove(string propertyName, object item)
        {
            ...
            {
                List<object> values;
                if (toSaveRelatedPropertyObjects.TryGetValue(propertyName, out values) && values.Contains(item))
                {
                    values.Remove(item);
                    isInSaveList = true;
                }
