...
namespace Rock.Orm.Common
{
    ...
    public class EntityArrayList<EntityType> : ICollection<EntityType>, IEntityArrayList, System.Collections.ICollection //,MarshalByRefObject, 
        where EntityType : Entity, new()
    {
        ...
        /// <returns></returns>
        public EntityType[] Filter(Rock.Orm.Common.WhereClip where, OrderByClip orderBy)
        {
            if (filter == null)
            {
                lock (this)
        ...
        {
            list.CopyTo((EntityType[])array, index);
        }
