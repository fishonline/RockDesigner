...
{
    [Designer(typeof(CountersignatureDesigner))]
    public sealed class Countersignature : NativeActivity<string>, IActivityTemplateFactory
    {
        [Description("工作流实例ID")]
        [Browsable(true)]
        ...
        {
            CountersignatureGuide customActivityGuide = new CountersignatureGuide();
            bool? result = customActivityGuide.ShowDialog();

            if (result == true)
            {
                Countersignature activity = new Countersignature();

                activity.DisplayName = "会签专用活动";
